...
using System.Linq; // For LINQ methods like Distinct() and Select()
...
namespace TCS.UIToolKitUtils {
    public class OdinToolKitHelperEditorWindow : OdinEditorWindow {
        ...
        GeneratorUtils m_staticClassGenerator;
        ...
        void InitializeComponents() {
            m_staticClassGenerator = new GeneratorUtils();
        ...
        void GenerateStyleSheetClass(string namespaceToUse, string filePath) {
            ...
            var classNames = ExtractClassNamesFromStyleSheet(m_styleSheet)
                .Distinct() // Ensure no duplicate class names
                .ToList();

            // Create a dictionary mapping variable names to original string values
            Dictionary<string, string> nameValuePairs = classNames
                .ToDictionary(
                    originalName => ConvertToConstName(originalName), // Variable name (identifier)
                    originalName => originalName                      // Original string value
                );

            m_staticClassGenerator.SaveToFile(namespaceToUse, className, nameValuePairs, filePath, m_maxLength);
    #endif
        }
        ...
        void GenerateUxmlClass(string namespaceToUse, string filePath) {
            ...
            var nameVariables = ExtractNamesFromUxml(m_visualTreeAsset)
                .Distinct() // Ensure no duplicate name variables
                .ToList();

            // Create a dictionary mapping variable names to original string values
            Dictionary<string, string> nameValuePairs = nameVariables
                .ToDictionary(
                    originalName => ConvertToConstName(originalName), // Variable name (identifier)
                    originalName => originalName                      // Original string value
                );
        ...
        // Method to convert names to uppercase with underscores (variable names)
        string ConvertToConstName(string name) {
            // Use Regex to split the name into words based on uppercase letters or numbers
            var words = Regex.Matches(name, @"[A-Z][a-z]*|[a-z]+|\d+")
                .Cast<Match>()
